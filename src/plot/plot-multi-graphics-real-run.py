...
for result_folder in folders:
    ...
    axs[0].set_ylabel('GPU IDs', fontsize=52)
    ...
    # axs[1].set_ylabel('Utility', fontsize=52)
    # axs[1].tick_params(labelsize=58)
    # axs[1].legend(prop={'size': 46}, loc="upper right")
    ...
    # axs[2].set_ylabel('Usage', fontsize=52)
    # axs[2].tick_params(labelsize=58)
    # axs[2].legend(prop={'size': 46}, loc="upper right")
    ...
    sns.tsplot(time=x, data=total_cpus, condition="GPU-CPU-GPU", color="b", linestyle="--", dashes=(10, 15), ax=axs[1])
    sns.tsplot(time=x, data=total_gpus, condition="P2P", color="g", linestyle="-", ax=axs[1])

    axs[1].xaxis.set_major_locator(matplotlib.ticker.MultipleLocator(48))
    axs[1].set_xlim([-0.5, window_max])
    axs[1].yaxis.set_major_locator(matplotlib.ticker.MultipleLocator(16))
    axs[1].set_ylim([-2, 60])

    axs[1].grid(b=True, which='major', color='w', linewidth=1.0)
    axs[1].grid(b=True, which='minor', color='w', linewidth=0.3)
    axs[1].set_ylabel('GB/s', fontsize=52)
    axs[1].tick_params(labelsize=58)
    axs[1].legend(prop={'size': 46})
    _ = [tick.label.set_fontsize(17) for tick in axs[1].xaxis.get_major_ticks()]

    if "P" in str(algo_name):
        axs[1].set_xlabel('Time (s)', fontsize=52)
    axs[1].tick_params(labelsize=58)
...
for time_type in ['exec_time', 'exec_time_submitted']:
    ...
    ax.legend(prop={'size': 46}, loc="lower right")
