...
def fcfs(curr_time, queue1, queue2, cluster_resources, profile, utility):
    ...
    while len(jobs) > 0:
        ...
        for mid, machine in temp_resources.machines.items():
            ...
            if machine.get_total_free_mem() >= mem and \
                ...
                for socket in machine.sockets:
                    if machine.get_free_mem_per_socket(socket.id) >= mem:
                        smem = mem
                    else:
                        smem = machine.get_free_mem_per_socket(socket.id)

                    if machine.get_free_core_per_socket(socket.id) >= task:
                        stask = task
                    else:
                        stask = machine.get_free_core_per_socket(socket.id)

                    if machine.get_free_gpu_per_socket(socket.id) >= num_gpus:
                        sgpu = num_gpus
                    else:
                        sgpu = machine.get_free_gpu_per_socket(socket.id)

                    # First, reduce the machine resources the requested resources
                    machine.alloc_mem_in_socket(socket.id, smem)
                    solution['mem_per_socket'][socket.id] += smem
                    mem -= smem

                    machine.alloc_core_in_socket(socket.id, stask)
                    solution['core_per_socket'][socket.id] += stask
                    task -= stask

                    # Randomly allocate the gpus
                    allocated_gpus = 1
                    for gpu in machine.sockets[socket.id].gpus:
                        if not gpu.allocated:
                            if allocated_gpus <= sgpu:
                                ...
                                pair["mid"] = mid
                                pair["job"] = job
                                solution['gpu_per_socket'][socket.id].append(pair)
                                allocated_gpus += 1
                            else:
                                break
                    num_gpus -= sgpu
        ...
        if best_placement is not None:
            ...
            # Wait for the first come job be placed
            if not best_placement.placement.placed:
                break
        else:
            break
